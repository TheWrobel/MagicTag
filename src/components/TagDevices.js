...
const TagDevices = ( {headers, deviceList, setLoading} ) => {
    ...
    const tagDevices = () => {
         deviceList.forEach(async el => {
            try {
                ...
                let reqData = await axios.put("http://192.168.42.21:7001/MagicInfo/restapi/v2.0/rms/devices/tags", taggingBody, {headers})
                console.log(reqData);
    ...
    return (
        <div className="tagDevices--content">
            ...
            {data ? <div>Tagged devices in session = {data}</div>: null}
        </div>
    )
}
